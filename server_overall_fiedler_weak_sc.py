    ...
    try:
        ...
        for trial in range(1, 4):
            ...
            for cycle in range(n_cycles):
                # 1. Inward Sweep (from both ends meeting in the middle)
                for res_idx in inward_order:
                    for joint in joints_by_res[res_idx]:
                        step_counter += 1
                        
                        best_angle_score = curr_score
                        ...
                        for d_theta in candidate_angles:
                            ...
                            # Contacts (no distance limit, different residues allowed to interact)
                            for i, j in zip(u_idx, v_idx):
                                if res_arr[i] == res_arr[j]:
                                    continue
                                d = dists_pep[i, j]
                                if is_polar[i] and is_polar[j]:
                                    ...
                                    if is_sc[i] and is_sc[j]:
                                        ...
                                        w *= 0.85
                                    G_pep_cand.add_edge(int(i), int(j), weight=float(w))
                                elif is_hydro[i] and is_hydro[j]:
                                    w = 40.0 / (d**2)
                                    # Weaker sidechain contacts
                                    if is_sc[i] and is_sc[j]:
                            ...
                            # COM attraction
                            com = np.mean(cand_coords, axis=0)
                            rg_hydro = np.mean(np.linalg.norm(cand_coords[hydro_indices] - com, axis=-1))
                            
                            cand_score = 30.0 * cand_f - 1.5 * rg_hydro
                ...
                # 2. Outward Sweep (from middle spreading out to both ends)
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        for d_theta in candidate_angles:
                            ...
                            for bond in peptide.bonds:
                                G_pep_cand.add_edge(int(bond["source"]), int(bond["target"]), weight=10.0)
                            dists_pep = np.linalg.norm(cand_coords[:, None, :] - cand_coords[None, :, :], axis=-1)
