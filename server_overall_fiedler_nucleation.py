...
def build_active_bond_graph(peptide_coords, water_coords, bonds, atom_types, is_sidechain, atom_residues, locked_contacts, contact_threshold_polar=4.0, contact_threshold_hydrophobic=4.5):
    G = nx.Graph()
    n_pep = len(peptide_coords)
    n_wat = len(water_coords)
    n_total = n_pep + n_wat
    ...
    # 2. Add Locked Contacts (Constraint Network)
    # The weight is dynamically calculated from the CURRENT distance to act as a spring constraint!
    ...
    # 4. Fluid Water Connections (Recalculated every step)
    wat_arr = np.array([w[0] for w in water_coords])
    ...
    # 5. Extract active peptide-peptide bonds for visualization
    active_hbonds = []
    active_sc_bonds = []
    active_bb_sc_bonds = []
...
def run_simulation_and_get_structure():
    try:
        ...
        print(f"\n========================================================")
        print(f"  Starting Solvent-Coupled NUCLEATION-PROPAGATION Folding")
            ...
            if os.path.exists(pdb_path):
                try:
                    ...
                    if pdb_mol:
                        ...
                        # Standard graph calculation for PDB targets
                        target_G, target_hb, target_sc, target_bb_sc, target_wb, target_wp = build_active_bond_graph(
                            folded_target, ideal_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, set()
                        )
        ...
        for trial in range(1, 4):
            ...
            # Initialize locked contacts set for this trial
            locked_contacts = set()
            
            G_total_init, act_hb, act_sc, act_bb_sc, act_wb, act_wp = build_active_bond_graph(
                curr_coords, water_coords, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts
            )
            ...
            step_counter = 0
            n_cycles = 10
            # Lightweight angle search (4 directions)
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        for d_theta in candidate_angles:
                            ...
                            G_cand, _, _, _, _, _ = build_active_bond_graph(
                                cand_coords, cand_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts
                            )
                        ...
                        # Lock newly formed stable contacts (Nucleation Phase-Locking)
                        # We evaluate contacts on the current established state
                        G_curr, curr_hb, curr_sc, curr_bb_sc, _, _ = build_active_bond_graph(
                            curr_coords, water_coords, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts
                        )
                        
                        # Polar contacts (backbone hydrogen bonds) locking
                        for i, j in curr_hb:
                            locked_contacts.add((i, j, "polar"))
                        # Hydrophobic contacts locking
                        for i, j in curr_sc:
                            locked_contacts.add((i, j, "hydro"))
                        # Backbone-sidechain contacts locking
            ...
            print(f"  Trial {trial} completed. Final Coupled Fiedler: {final_fiedler:.6f} | Final Rg: {final_rg:.3f} Å | Locked Contacts: {len(locked_contacts)}")
        ...
        G_final, _, _, _, _, _ = build_active_bond_graph(
            final_coords_centered, centered_final_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, set()
        )
        ...
        print("\n========================================================")
        print(f"  FINAL NETWORK GRAPH SIMULATION RESULTS (NUCLEATION)")
        print(f"--------------------------------------------------------")
