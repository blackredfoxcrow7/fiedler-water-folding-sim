...
def build_active_bond_graph(peptide_coords, water_coords, bonds, atom_types, is_sidechain, atom_residues, contact_threshold_polar=4.0, contact_threshold_hydrophobic=4.5):
    ...
    # Non-covalent peptide internal contacts (no distance limit, different residues allowed to interact)
    for i, j in zip(u_idx, v_idx):
        ...
        if is_polar[i] and is_polar[j]:
            w = 16.0 / (d**2)
            # Weaker sidechain contacts
            if is_sc[i] and is_sc[j]:
                w *= 0.70
            elif is_sc[i] or is_sc[j]:
                w *= 0.85
            ...
            for cycle in range(n_cycles):
                ...
                for res_idx in inward_order:
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        # We evaluate Fiedler value for each candidate angle
                        for d_theta in candidate_angles:
                            ...
                            # Identify local waters within 6.0 Å of current candidate coordinates
                            wat_O_coords = np.array([w[0] for w in water_coords])
                            dists_wat_pep = np.linalg.norm(cand_coords[None, :, :] - wat_O_coords[:, None, :], axis=-1) # shape (n_wat, n_pep)
                            local_water_mask = np.any(dists_wat_pep < 6.0, axis=-1)
                            local_water_indices = np.where(local_water_mask)[0]
                            n_local_wat = len(local_water_indices)
                            ...
                            for target_res_idx in range(peptide.n_residues):
                                ...
                                # Build G_comb containing: Peptide + Local Waters + Polar-Water edges
                                G_comb = nx.Graph()
                                n_nodes = peptide.n_atoms + n_local_wat
                                G_comb.add_nodes_from(range(n_nodes))
                                
                                # Peptide Covalent bonds
                                for covalent_bond in peptide.bonds:
                                    G_comb.add_edge(int(covalent_bond["source"]), int(covalent_bond["target"]), weight=10.0)
                                    
                                # Peptide Accumulated bonds
                                        ...
                                        for p in atoms_b:
                                            ...
                                            if is_polar[i] and is_polar[p]:
                                                ...
                                                if is_sc[i] and is_sc[p]:
                                                    ...
                                                    w *= 0.85
                                                G_comb.add_edge(int(i), int(p), weight=float(w))
                                ...
                                # Water-Water edges within local waters
                                for u_local_idx, u in enumerate(local_water_indices):
                                    pos_u = wat_O_coords[u]
                                    for v_local_idx, v in enumerate(local_water_indices):
                                        if u_local_idx >= v_local_idx:
                                            continue
                                        d_wat = np.linalg.norm(pos_u - wat_O_coords[v])
                                        if d_wat < 3.5:
                                            w = max(0.1, 3.5 - d_wat)
                                            G_comb.add_edge(peptide.n_atoms + u_local_idx, peptide.n_atoms + v_local_idx, weight=float(w))
                                            
                                # Polar Peptide-Water edges
                                for u_local_idx, u in enumerate(local_water_indices):
                                    # distance between all peptide polar atoms and water u
                                    for p_idx in np.where(is_polar)[0]:
                                        d_pw = dists_wat_pep[u, p_idx]
                                        if d_pw < 3.0:
                                            w = max(0.1, 3.0 - d_pw)
                                            G_comb.add_edge(int(p_idx), peptide.n_atoms + u_local_idx, weight=float(w))
                                            
                                # Calculate Combined Fiedler (Positive)
                                f_comb, _ = get_network_metrics(G_comb)
                                
                                # Calculate Hydrophobic-Water penalty (Negative)
                                hydro_penalty = 0.0
                                for u in local_water_indices:
                                    for h_idx in hydro_indices:
                                        d_hw = dists_wat_pep[u, h_idx]
                                        if d_hw < 4.5:
                                            hydro_penalty += 40.0 / (d_hw**2)
                                            
                                # Neg-Pos Duality Score
                                cand_score = f_comb - 1.5 * hydro_penalty
                                
                                if cand_score > best_angle_score:
                                    best_angle_score = cand_score
                                    best_angle_coords = cand_coords
                                    best_angle_f = f_comb
                                    best_target_res_idx = target_res_idx
                                    
                        # Apply best rotation and keep the chosen bond permanently
                        if best_angle_coords is not None:
                            curr_coords = best_angle_coords
                            # Evaluate peptide only Fiedler for initial display
                            G_pep_only = nx.Graph()
                            G_pep_only.add_nodes_from(range(peptide.n_atoms))
                            for covalent_bond in peptide.bonds:
                                G_pep_only.add_edge(int(covalent_bond["source"]), int(covalent_bond["target"]), weight=10.0)
                            ...
                            for r_a, r_b in accumulated_bonds:
                                ...
                                for i in atoms_a:
                                    for p in atoms_b:
                                        ...
                                        if is_polar[i] and is_polar[p]:
                                            w = 16.0 / (d**2)
                                            G_pep_only.add_edge(int(i), int(p), weight=float(w))
                                        elif is_hydro[i] and is_hydro[p]:
                                            w = 40.0 / (d**2)
                                            G_pep_only.add_edge(int(i), int(p), weight=float(w))
                            p_f_init, _ = get_network_metrics(G_pep_only)
                            ...
                            curr_score = 30.0 * p_f_init - 1.5 * rg_hydro_c
                ...
                for res_idx in outward_order:
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        for d_theta in candidate_angles:
                            ...
                            # Identify local waters within 6.0 Å
                            wat_O_coords = np.array([w[0] for w in water_coords])
                            dists_wat_pep = np.linalg.norm(cand_coords[None, :, :] - wat_O_coords[:, None, :], axis=-1)
                            local_water_mask = np.any(dists_wat_pep < 6.0, axis=-1)
                            local_water_indices = np.where(local_water_mask)[0]
                            n_local_wat = len(local_water_indices)
