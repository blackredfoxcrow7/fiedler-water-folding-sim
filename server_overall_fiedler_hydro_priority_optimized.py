...
class PeptideAgent:
    def __init__(self, sequence):
        ...
        # Add hydrogens and generate conformer with fast ETKDGv3 parameters
...
def generate_extended_coords(peptide):
    """Generates coordinates for a fully extended (unfolded) peptide backbone conformer."""
    coords = peptide.initial_coords.copy()
    
    # We rotate joints sequentially to straighten the chain out (anti-conformation)
    for joint in peptide.joints:
        u_idx = joint["u_idx"]
        d_idx = joint["d_idx"]
        D = joint["downstream_atoms"]
        
        # Align rotatable bond with standard axis
        pos_A = coords[u_idx]
        pos_B = coords[d_idx]
        axis = pos_B - pos_A
        ...
        if axis_len > 1e-5:
            axis /= axis_len
            # Force trans dihedral (approx 180 degrees)
...
def generate_h2o_coords(pos_O):
    """Generates a planar, rigid water molecule geometry centered at Oxygen pos_O."""
    pos_O = np.array(pos_O)
    
    # Random orthogonal vectors for H-H placement
    ...
    if v2_len > 1e-5:
        ...
        v2 = np.cross(v1, [0.0, 1.0, 0.0])
        
    angle_rad = np.radians(52.25) # half of 104.5 degrees
...
def update_solvent_physics(peptide_coords, water_coords, atom_types):
    """
    Simulates solute-solvent forces on water molecules:
    1. Electrostatic attraction to polar peptide atoms.
    2. Hydrophobic repulsion from hydrophobic residues (preventing water penetration into hydrophobic core).
    3. Water-water hydrogen bond networks.
    """
    ...
    d_pw = wat_arr[:, None, :] - pep_arr[None, :, :] # shape (N_w, N_p, 3)
    dist_pw = np.linalg.norm(d_pw, axis=-1)
    dist_pw = np.maximum(dist_pw, 0.5)
    
    # Polar attraction (Lennard-Jones-like attraction toward polar peptide atoms)
    F_polar = -1.5 * d_pw / (dist_pw**3)[:, :, None]
    # Hydrophobic repulsion (pushing water out of the core)
...
def build_active_bond_graph(peptide_coords, water_coords, bonds, atom_types, is_sidechain, atom_residues, contact_threshold_polar=4.0, contact_threshold_hydrophobic=4.5):
    ...
    # Non-covalent peptide internal contacts
    ...
    # Keep at most 4 active connections per water
    active_water_bonds = []
...
def run_simulation_and_get_structure():
    try:
        ...
        print(f"\n========================================================")
        print(f"  Starting Solvent-Coupled OVERALL Fiedler Folding (OPTIMIZED)")
        ...
        # Initialize unfolded water coordinates
        unfolded_waters = []
        ...
        # 3. Optimization Loop with Multi-Trial Restarts
        successful = False
        trajectory = []
            ...
            for cycle in range(n_cycles):
                ...
                for res_idx in inward_order:
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        # We evaluate Fiedler value once per candidate angle (combining all valid contacts)
                        for d_theta in candidate_angles:
                            ...
                            # Build combined candidate graph
                            G_pep_cand = nx.Graph()
                            ...
                            # Add all non-covalent contacts formed in this candidate coords conformation
                            ...
                            # Add accumulated bonds explicitly
                        ...
                        if best_angle_coords is not None:
                            ...
                            # Identify the closest contact to add to accumulated_bonds
