...
const BACKEND_URL = `http://${BACKEND_HOST}:5000`;
...
export default function App() {
  ...
  const [waterWaterStrength, setWaterWaterStrength] = useState(0.35);
  const [waterPeptideStrength, setWaterPeptideStrength] = useState(0.40);
  const [bulkSolventStrength, setBulkSolventStrength] = useState(0.0);
  const [waterBeltContraction, setWaterBeltContraction] = useState(0.0);
  const [watersPerAtom, setWatersPerAtom] = useState(2);
      ...
      <div className="app-body">
        ...
        <div className={`panel-sidebar ${showSidebar ? '' : 'collapsed'}`}>
          ...
          <div className="glass-panel" style={{ padding: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
                <span className="slider-title" style={{ fontSize: '12px', color: '#94a3b8' }}>水分子の描画（表示）</span>
                <button
                  onClick={() => setShowWaterVisuals(!showWaterVisuals)}
                  className={showWaterVisuals ? "primary" : "secondary"}
                  style={{
                ...
                <>
                  ...
                  <div className="slider-group">
                    <div className="slider-header">
                      <span className="slider-title">水の帯の結合強度 (K_ww)</span>
                    ...
                    />
                    <span className="slider-desc">水粒子同士が結合して「水の帯」を形成する引力の強さ。</span>
                  </div>

                  {/* Slider: Water Belt Contraction */}
                  <div className="slider-group" style={{ marginTop: '12px' }}>
                    <div className="slider-header">
                      <span className="slider-title">水の帯の収縮率 (K_contract)</span>
                    ...
                    <input
                      ...
                      className="slider-cyan"
                    />
                    <span className="slider-desc">水の帯をなげなわのように縮めて絞り込む比率（0.0で通常、大きいほど縮小）。</span>
                  </div>

                  {/* Slider: Water Molecules Per Atom */}
                  <div className="slider-group" style={{ marginTop: '12px' }}>
                    <div className="slider-header">
                      <span className="slider-title">親水基あたりの水分子数</span>
                      <span className="slider-value cyan" style={{ color: '#22d3ee' }}>{watersPerAtom} 分子</span>
                    </div>
                    <input
                      type="range"
                      min="2"
                      ...
                      step="1"
                      value={watersPerAtom}
                      onChange={(e) => updateParameter('watersPerAtom', parseInt(e.target.value))}
                      className="slider-cyan"
                    />
                    <span className="slider-desc">各親水基の周りに配置する水分子の数（変更時に水分子が再配置されます）。</span>
                  </div>

                  {/* Slider: Water-Peptide Strength */}
                  <div className="slider-group">
                    <div className="slider-header">
                      <span className="slider-title">水-親水基の引力強度 (K_wp)</span>
                      <span className="slider-value blue" style={{ color: '#60a5fa' }}>{waterPeptideStrength.toFixed(2)}</span>
                    </div>
                    <input
                      type="range"
                      min="0.05"
                    ...
                    />
                    <span className="slider-desc">水粒子がペプチド親水性残基の親水基を引き寄せる引力の強さ。</span>
                  </div>
                </>
              )}
              ...
              <div className="slider-group" style={{ marginTop: '12px' }}>
                <div className="slider-header">
                  <span className="slider-title">バルク溶媒圧効果 (K_solv)</span>
                  <span className="slider-value orange" style={{ color: '#fb923c' }}>{bulkSolventStrength.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.00"
                ...
                />
                <span className="slider-desc">周囲の無限のバルク水から受ける圧力。疎水基を内部へ押し込み、親水基を外側に露出させて「疎水性コア」と「親水性外殻」への分離を促します。</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel" style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <h2 className="panel-title">
              <Sliders className="w-4 h-4 text-cyan-400" /> 溶媒エージェントの力を制御
            </h2>
            ...
            <div className="slider-group">
              <div className="slider-header">
                <span className="slider-title">
                  <Thermometer className="w-3.5 h-3.5 text-blue-400" /> 🌡️ 溶媒温度 (ゆらぎ/ルールC)
                </span>
              ...
              <span className="slider-desc">
                全原子にランダムな力（ホワイトノイズ）を加え、分子の熱ゆらぎをシミュレートします。
              </span>
            </div>
