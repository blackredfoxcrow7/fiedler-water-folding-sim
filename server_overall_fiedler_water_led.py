...
def get_water_only_fiedler(G, n_pep, n_wat):
    """Calculates Fiedler connectivity of the largest connected component of the water-only subgraph."""
    try:
        water_nodes = list(range(n_pep, n_pep + n_wat))
        G_water = G.subgraph(water_nodes)
        if G_water.number_of_nodes() <= 1:
            return 0.0
        
        components = list(nx.connected_components(G_water))
        if not components:
            return 0.0
        largest_comp = max(components, key=len)
        G_sub = G_water.subgraph(largest_comp)
...
def run_simulation_and_get_structure():
    try:
        ...
        print(f"\n========================================================")
        print(f"  Starting WATER-LED (Solvent-Only Fiedler) Folding")
            ...
            if os.path.exists(pdb_path):
                try:
                    ...
                    if pdb_mol:
                        ...
                        ideal_fiedler = get_water_only_fiedler(target_G, peptide.n_atoms, len(ideal_waters))
                        ideal_spectral_entropy = 2.85
        ...
        print(f"Target Ideal Water Fiedler: {ideal_fiedler:.6f} | Target Radius of Gyration: {target_rg:.3f} Å")
        ...
        for trial in range(1, 4):
            ...
            water_f_init = get_water_only_fiedler(G_total_init, peptide.n_atoms, len(water_coords))
            ...
            curr_score = 30.0 * water_f_init - 1.5 * rg_hydro_init
            
            step_counter = 0
            n_cycles = 10
            # Lightweight angle list: only 4 candidates (2x speedup)
                ...
                for res_idx in inward_order:
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        for d_theta in candidate_angles:
                            ...
                            # Only 2 steps of water relaxation per angle (33% speedup)
                            for _ in range(2):
                            ...
                            cand_f = get_water_only_fiedler(G_cand, peptide.n_atoms, len(cand_waters))
                        ...
                        if step_counter % 8 == 0:
                            ...
                            w_f_acc = get_water_only_fiedler(G_cand, peptide.n_atoms, len(water_coords))
                            trial_traj.append({
                                "step": len(trial_traj),
            ...
            final_fiedler = trial_traj[-1]["waterFiedler"] if trial_traj else water_f_init
            final_rg = np.sqrt(np.mean(np.sum((curr_coords - np.mean(curr_coords, axis=0))**2, axis=1)))
            print(f"  Trial {trial} completed. Final Water Fiedler: {final_fiedler:.6f} | Final Rg: {final_rg:.3f} Å | Score: {curr_score:.3f}")
        ...
        print(f"✓ Completed all 3 trials. Selecting the best trial conformation (Water Fiedler = {best_fiedler:.6f}, Rg = {final_rg_sel:.3f} Å, Score = {best_score:.3f}).")
        ...
        w_f_ref = get_water_only_fiedler(G_final, peptide.n_atoms, len(centered_final_waters))
        
        print("\n========================================================")
        print(f"  FINAL NETWORK GRAPH SIMULATION RESULTS (WATER-LED)")
        print(f"--------------------------------------------------------")
        print(f"  UNFOLDED STATE (Step 0):")
        print(f"    Water Fiedler:   {water_f_init:.6f}")
