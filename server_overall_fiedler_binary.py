...
def build_active_bond_graph(peptide_coords, water_coords, bonds, atom_types, is_sidechain, atom_residues, contact_threshold_polar=4.0, contact_threshold_hydrophobic=4.5):
    """Builds the active bond graph based on binary 1.0 or 0.0 contact rules."""
    G = nx.Graph()
    n_pep = len(peptide_coords)
    n_wat = len(water_coords)
    n_total = n_pep + n_wat
    ...
    # 1. Covalent bonds (weight = 1.0)
    for bond in bonds:
        G.add_edge(int(bond["source"]), int(bond["target"]), weight=1.0)
        
    # 2. Peptide internal contacts (weight = 1.0)
    pep_arr = np.array(peptide_coords)
    ...
    for i, j in zip(u_idx, v_idx):
        ...
        if is_polar[i] and is_polar[j] and d < 4.0:
            G.add_edge(int(i), int(j), weight=1.0)
        elif is_hydro[i] and is_hydro[j] and d < 4.5:
            G.add_edge(int(i), int(j), weight=1.0)
        
    # 3. Water-water and water-peptide contacts (weight = 1.0)
    wat_arr = np.array([w[0] for w in water_coords])
    ...
    for u in range(n_wat):
        ...
        for neighbor, d in candidates[:4]:
            G.add_edge(int(n_pep + u), int(neighbor), weight=1.0)
            if neighbor >= n_pep:
                v = neighbor - n_pep
                if u < v:
                        ...
                        for d_theta in candidate_angles:
                            ...
                            for target_res_idx in range(peptide.n_residues):
                                ...
                                for covalent_bond in peptide.bonds:
                                    G_pep_cand.add_edge(int(covalent_bond["source"]), int(covalent_bond["target"]), weight=1.0)
                                ...
                                for i in active_atoms:
                                    for p in target_atoms:
                                        d = dists_pep[i, p]
                                        if is_polar[i] and is_polar[p] and d < 4.0:
                                            G_pep_cand.add_edge(int(i), int(p), weight=1.0)
                                        elif is_hydro[i] and is_hydro[p] and d < 4.5:
                                            G_pep_cand.add_edge(int(i), int(p), weight=1.0)
