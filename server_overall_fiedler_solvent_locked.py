...
def build_active_bond_graph(peptide_coords, water_coords, bonds, atom_types, is_sidechain, atom_residues, locked_contacts, locked_water_bonds, locked_water_peptide_bonds, locked_ww_arr, locked_wp_arr, contact_threshold_polar=4.0, contact_threshold_hydrophobic=4.5):
    G = nx.Graph()
    n_pep = len(peptide_coords)
    n_wat = len(water_coords)
    n_total = n_pep + n_wat
    ...
    # 1. Covalent bonds
    G.add_edges_from([(int(b["source"]), int(b["target"]), {"weight": 10.0}) for b in bonds])
    ...
    # 2. Add Locked Peptide-Peptide Contacts
    for i, j, c_type in locked_contacts:
        d = dists_pep[i, j]
        w = 16.0 / (d**2) if c_type == "polar" else 40.0 / (d**2)
        G.add_edge(int(i), int(j), weight=float(w))
        
    # 3. Add Dynamic peptide-peptide contacts
    u_idx, v_idx = np.triu_indices(n_pep, k=1)
    for i, j in zip(u_idx, v_idx):
        if res_arr[i] == res_arr[j]:
            continue
        if (i, j, "polar") in locked_contacts or (i, j, "hydro") in locked_contacts:
            continue
        d = dists_pep[i, j]
    ...
    # 4. Add Locked Water-Water and Water-Peptide Bonds (Vectorized!)
    # We only add edges for locked bonds that are CURRENTLY within active hydrogen bonding distance,
    # keeping the graph sparse and avoiding O(N^2) dense Laplacian matrix slowdowns.
    if n_wat > 0:
        if locked_ww_arr is not None and len(locked_ww_arr) > 0:
            dists_ww = np.linalg.norm(wat_arr[:, None, :] - wat_arr[None, :, :], axis=-1)
            valid = (locked_ww_arr[:, 0] < n_wat) & (locked_ww_arr[:, 1] < n_wat)
            v_idx = locked_ww_arr[valid]
            if len(v_idx) > 0:
                ds = dists_ww[v_idx[:, 0], v_idx[:, 1]]
                active = ds < 3.5
                v_idx = v_idx[active]
                if len(v_idx) > 0:
                    ws = 3.5 - ds[active]
                    G.add_edges_from([(int(n_pep + u), int(n_pep + v), {"weight": float(w)}) for (u, v), w in zip(v_idx, ws)])
                
        if locked_wp_arr is not None and len(locked_wp_arr) > 0:
            dists_pw = np.linalg.norm(pep_arr[:, None, :] - wat_arr[None, :, :], axis=-1)
            valid = (locked_wp_arr[:, 0] < n_wat) & (locked_wp_arr[:, 1] < n_pep)
            v_idx = locked_wp_arr[valid]
            if len(v_idx) > 0:
                ds = dists_pw[v_idx[:, 1], v_idx[:, 0]]
                active = ds < 3.0
                v_idx = v_idx[active]
                if len(v_idx) > 0:
                    ws = 3.0 - ds[active]
                    G.add_edges_from([(int(n_pep + u), int(v), {"weight": float(w)}) for (u, v), w in zip(v_idx, ws)])

    # 5. Find new dynamic water connections
    ...
    if n_wat > 0:
        ...
        for idx in range(len(pep_indices)):
            ...
            if (u, i) not in locked_water_peptide_bonds:
        ...
        for idx in range(len(u_indices)):
            u = int(u_indices[idx])
            v = int(v_indices[idx])
            if (u, v) not in locked_water_bonds and (v, u) not in locked_water_bonds:
    ...
    # Compile active lists for visualization
    for i, j in G.edges():
        ...
        elif (i >= n_pep and j < n_pep) or (j >= n_pep and i < n_pep):
            u = (i - n_pep) if i >= n_pep else (j - n_pep)
            neighbor = j if i >= n_pep else i
...
def run_simulation_and_get_structure():
    try:
        ...
        print(f"\n========================================================")
        print(f"  Starting Solvent-Coupled LOCKED-SOLVENT Folding")
                ...
                try:
                    ...
                    if pdb_mol:
                        ...
                        target_G, target_hb, target_sc, target_bb_sc, target_wb, target_wp = build_active_bond_graph(
                            folded_target, ideal_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, set(), set(), set()
                        )
        ...
        for trial in range(1, 4):
            ...
            # Initialize locked lists for this trial
            locked_contacts = set()
            locked_water_bonds = set()
            locked_water_peptide_bonds = set()
            
            G_total_init, act_hb, act_sc, act_bb_sc, act_wb, act_wp = build_active_bond_graph(
                curr_coords, water_coords, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts, locked_water_bonds, locked_water_peptide_bonds, None, None
            )
            ...
            step_counter = 0
            n_cycles = 10
            # Optimized 4 candidate directions
            ...
            for cycle in range(n_cycles):
                ...
                for res_idx in inward_order:
                    ...
                    for joint in joints_by_res[res_idx]:
                        ...
                        # Pre-convert to numpy arrays for massive speedup!
                        locked_ww_arr = np.array(list(locked_water_bonds), dtype=np.int32) if locked_water_bonds else None
                        locked_wp_arr = np.array(list(locked_water_peptide_bonds), dtype=np.int32) if locked_water_peptide_bonds else None
                        ...
                        for d_theta in candidate_angles:
                            ...
                            G_cand, _, _, _, _, _ = build_active_bond_graph(
                                cand_coords, cand_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts, locked_water_bonds, locked_water_peptide_bonds, locked_ww_arr, locked_wp_arr
                            )
                        ...
                        # Relax water coordinates once at the end of the step
                        for _ in range(3):
                            water_coords = update_solvent_physics(curr_coords, water_coords, peptide.atom_types)
                            
                        # Update arrays for locking step
                        locked_ww_arr = np.array(list(locked_water_bonds), dtype=np.int32) if locked_water_bonds else None
                        locked_wp_arr = np.array(list(locked_water_peptide_bonds), dtype=np.int32) if locked_water_peptide_bonds else None
                        
                        G_curr, curr_hb, curr_sc, curr_bb_sc, curr_wb, curr_wp = build_active_bond_graph(
                            curr_coords, water_coords, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, locked_contacts, locked_water_bonds, locked_water_peptide_bonds, locked_ww_arr, locked_wp_arr
                        )
                        for i, j in curr_hb:
                            locked_contacts.add((i, j, "polar"))
                        for i, j in curr_sc:
                            locked_contacts.add((i, j, "hydro"))
                        for i, j in curr_bb_sc:
                            locked_contacts.add((i, j, "polar"))
                        for u, v, h_idx in curr_wb:
                            locked_water_bonds.add((u, v))
                        for u, neighbor, t in curr_wp:
                            locked_water_peptide_bonds.add((u, neighbor))
            ...
            print(f"  Trial {trial} completed. Final Coupled Fiedler: {final_fiedler:.6f} | Final Rg: {final_rg:.3f} Å | Locked Waters: {len(locked_water_bonds)}")
        ...
        print(f"✓ Completed all 3 trials. Selecting the best trial conformation (Coupled Fiedler = {best_fiedler:.6f}, Rg = {final_rg_sel:.3f} Å).")
        ...
        G_final, _, _, _, _, _ = build_active_bond_graph(
            final_coords_centered, centered_final_waters, peptide.bonds, peptide.atom_types, peptide.is_sidechain, peptide.atom_residues, set(), set(), set(), None, None
        )
        ...
        print("\n========================================================")
        print(f"  FINAL NETWORK GRAPH SIMULATION RESULTS (LOCKED-SOLVENT)")
        print(f"--------------------------------------------------------")
